...
(deftest message-handler-test
  (let [link "https://example.com/link"
    ...
    (with-redefs [download-file (fn [url _]
                                  (is (= url link))
                                  "mock-location")
                  send-telegram-command (let [expected-arguments (atom [{:method "sendChatAction" :action "upload_video"}
    ...
    (testing "Unsuccessful send"
      (with-redefs [download-file (fn [url _]
                                    (is (= url link))
                                    false)
                    send-telegram-command (let [expected-arguments (atom [{:args {:method "sendChatAction" :action "upload_video"} :resp 200}
...
(comment
  (let [link "https://example.com/link"
        chat-id 123
        message-id 321
        reply-to-id 456] (format "{
  \"link\": \"%s\",
  \"chat-id\": %d,
  \"message-id\": %d,
  \"reply-to-id\": %d
  }" link chat-id message-id reply-to-id))
  (clojure.set/subset? {:kissa 123} {:kissa 123 :asdf 321})
  (run-tests))
